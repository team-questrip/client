...
const createResponseData = (page: number, hasNext: boolean) => ({
  status: 'SUCCESS',
  message: null,
  data: {
    content: Array.from({ length: 10 }, createContent),
    page,
    size: 10,
    numberOfElements: 10,
    hasNext,
...
mock.onGet(`api/v1/recommend/ACCEPTED`).reply(200, createResponseData(0, true));

mock
  .onGet(`api/v1/recommend/ACCEPTED?page=0`)
  .reply(200, createResponseData(0, true));

mock
  .onGet(`api/v1/recommend/ACCEPTED?page=1`)
  .reply(200, createResponseData(1, false));
